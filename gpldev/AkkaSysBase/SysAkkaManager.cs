...
namespace AkkaSysBase
{
    ...
    {
        private Lazy<ActorSystem> LazyActorSystem { get; set; }

        public ActorSystem ActorSystem => LazyActorSystem.Value;

        private  readonly Dictionary<string, IActorRef> _actorDics = new Dictionary<string, IActorRef>();

        public SysAkkaManager(Lazy<ActorSystem> lazyActorSystem)
        {
            LazyActorSystem = lazyActorSystem;
        }
