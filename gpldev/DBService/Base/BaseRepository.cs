...
namespace DBService.Base
{
    public abstract class BaseRepository<T>  : IRepository<T> 
     ...
     {    
        ...
        public virtual bool HasDummy(string[] pkValue)
        {
            var where = SetPkWhereSql(pkValue);
            var sql = $"select count(1) from {TableName} WHERE {where} AND Order_No = 'DUMMY' ";
            return DBContext.ExecuteScalar<bool>(sql);
        }
        public virtual bool DeleteDummy(string[] pkValue)
        {
            var where = SetPkWhereSql(pkValue);
            var sql = $"DELETE from {TableName} WHERE {where} AND Order_No = 'DUMMY' ";
            return DBContext.ExecuteScalar<bool>(sql);
        }
